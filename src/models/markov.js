...
class Markov {
    ...
    constructor (order, entries) {
        ...
        const entriesForEachOrder = _.flatten(_.map(entries, entry => {
            return _.reduce (entry.previous, (orderedEntryAccumulator, previous) => {
                orderedEntryAccumulator.push({
                    next: entry.next,
                    previous: [..._.get(_.last(orderedEntryAccumulator), 'previous', []), previous]
                });
        ...
        this._entriesMap = _.reduce(entriesForEachOrder, (allEntries, entry) => {
    ...
    getEntriesGivenThreshold(previousStates, selectionThreshold=1) {
        ...
        return _.findLast(entriesForAllOrders, entries => entries.length >= selectionThreshold) || _.first(entriesForAllOrders);
    }
}
